...
Computes deramped phase, phi, for the given burst number for lines and samples.
...
# Output
- phi::Array{Float64,2}: Array with deramped phase for line and sample
...
function phase_ramp(lines, samples, burst_number, meta, v_mid)
    ...
    eta = -lines_per_burst/2*Delta_t_s .+ (lines .- 1) .* Delta_t_s
    
    phase = pi * k_t .* (eta .- eta_ref).^2 .+ 2 * pi .* f_etac .* (eta .- eta_ref); # Eqn. 14
    return phase
end
...
# # Arguments
# - `lut::Dict`:  The look up table in the form of a dictionary 
# - `burst_number::Int`: The number of the burst of interest.
# - `meta::Dict`:  Meta information from the Sentinel-1 SLC image
# - `precise_orbit:Dict`: Precise orbit data
# 
# # Examples:
# ```jldoctest
# julia> phi_reramp = reramp(lut, burst_number, meta, precise_orbit)
# julia> reramped = deramped_data .* phi_reramp
# ```
# """
# function reramp(lut, burst_number, meta, precise_orbit_data)
#     dline = mean(lut["slave_line"] .- lut["master_line"])
#     dsample = mean(lut["slave_sample"] .- lut["master_sample"])
#     reramped_phase = deramp(lut["master_line"] .+ dline, lut["master_sample"] .+ dsample, burst_number, meta, precise_orbit)
#     phi = exp.(im .* reramped_phase)
#     return phi
# end
